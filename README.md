...
本編は**運営代行のみ**。3プラン（ライト10% / ミドル18% / フル20%）。
入口は無料相談（Googleフォーム）。
...
| 複数棟割引 | JS内 `DISCOUNT = { 1:0, 5:1, 10:2 }` と `.sim__tabs--units` の表記。割引は全棟に適用 |
| OTA手数料率 | JS内 `OTA_RATE = 0.155`（FAQの試算例と分岐点も要連動） |
| 駆けつけの提供状況 | 比較表「現地駆けつけ対応」の行、`#scope` のオプション |
| 実績数値 | `#result` セクション |
| オプションの内容 | `#scope` の `.scope__opt`（9項目。**ここが唯一のオプション一覧**。別セクションを作らないこと） |
