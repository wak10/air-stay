...
支給された `AirStayロゴ.svg`（1254×1254）をWeb用に加工した3種類を使用。

| ファイル | 用途 | 比率 |
|---|---|---|
| `assets/brand/airstay-logo-h.svg` | ナビ（デスクトップ） | 7.5 : 1 |
| `assets/brand/airstay-logo.svg` | フッター（縦組み） | 1.9 : 1 |
| `assets/brand/airstay-mark.svg` | ナビ（モバイル）・狭い幅 | 2.6 : 1 |
| `assets/brand/airstay-mark-coral.svg` | ファビコン | — |

**加工内容**（原本をそのまま使うと破綻します）
- `viewBox` を追加（原本には無く拡大縮小できない）
- 全面の不透明な白背景パスを削除
- 余白をトリミング（原本はロゴが中央の一部のみ）
- 「A」「a」のカウンター（穴）を **`<mask>` で透明化**。原本は白パスの上塗りで、
  ダーク背景に置くと穴が白く残る
- 座標を小数1桁に丸めて 74KB → 35KB

**色の変え方**：インクは `#241A16` 固定。白抜きは CSS の
`filter: brightness(0) invert(1)` で行う（フッターがこの方式）。
穴は透明なので背景色に追従します。
